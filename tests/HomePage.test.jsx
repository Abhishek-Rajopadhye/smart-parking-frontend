...
jest.mock('axios'); // Mock axios calls if any are made by child components or HomePage itself
jest.mock('../../components/SearchBar', () => {
    return ({ searchAddress, handleSearchChange, handleClearSearch, handleUseMyLocation, handleSuggestionClick, predictions, suggestions, isMobile, myLocationstatus, mtLocationMessage }) => (
        <div>
            <input
            ...
            />
            <button data-testid="clear-search-button" onClick={handleClearSearch}>Clear</button>
            <button data-testid="my-location-button" onClick={handleUseMyLocation}>Use My Location</button>
...
jest.mock('../../components/NearByParkings', () => {
  return ({ isMobile, origin, onSpotSelect }) => (
    <div data-testid="nearby-parkings">Nearby Parkings</div>
  );
});

jest.mock('../../components/PastBooking', () => {
    return ({ user, isMobile }) => (
      <div data-testid="past-booking">Past Booking</div>
    );
  });
...
const mockSetNewMarker = jest.fn();
...
describe('HomePage', () => {
                ...
                maps: {
                    places: {
                        AutocompleteService: jest.fn(() => ({
                            getPlacePredictions: jest.fn((options, callback) => {
                    ...
                    Geocoder: jest.fn(() => ({
                        geocode: jest.fn((options, callback) => {
                            if (options.address === 'Test Address, City') {
                                ...
                                callback([{ formatted_address: 'Mocked Current Location' }], 'OK');
                            }
                             else {
                                callback([], 'ZERO_RESULTS');
                            }
    ...
    afterEach(() => {
        jest.spyOn(globalThis.navigator, 'geolocation').mockRestore();
        ...
        render(
            <MapContext.Provider value={{ isLoaded: false, loadError: null }}>
                 <AuthContext.Provider value={{ user: mockUser }}>
                    <HomePage setSelectedMarker={mockSetSelectedMarker} setNewMarker={mockSetNewMarker} setFilters={mockSetFilters} />
    ...
    test('shows alert if Find Parking Spots button is clicked with invalid address', async () => {
        ...
        await waitFor(() => {
            expect(mockAlert).toHaveBeenCalledWith('Please enter a valid location to search for parking spots.');
        });
