...
const UserBookingView = ({ bookingDetails, cancelBooking, checkIn, checkOut }) => {
    ...
    const handleBookAgain = async (spotId, booking) => {
        try {
            const response = await axios.get(`${BACKEND_URL}/spots/${spotId}`);
    ...
    return (
        <Box sx={{ padding: 1 }}>
			...
			<Booking open={bookAgainOpen} set_dialog={toggleBookAgainOpen} previous_booking={previousBooking} spot_information={spotInformation} />
