...
const MapSidebar = ({ mapRef, setNewMarker, setSelectedMarker, markers, setFilters, filteredMarkers }) => {
	...
	const handleUpdateSearch = () => {
		...
		if (tempStartTime) {
			setFilters((prev) => ({
				...prev,
				open_time: (tempStartTime?.getHours() || 0) + ":" + tempStartTime?.getMinutes().toString().padStart(2, "0"),
			}));
			console.log(
				"Enter After:",
				(tempStartTime?.getHours() || 0) + ":" + tempStartTime?.getMinutes().toString().padStart(2, "0")
			);
		}
		...
		if (tempEndTime) {
			setFilters((prev) => ({
				...prev,
				close_time: (tempEndTime?.getHours() || 0) + ":" + tempEndTime?.getMinutes().toString().padStart(2, "0"),
			}));
			console.log(
				"Exit Before:",
				(tempEndTime?.getHours() || 0) + ":" + tempEndTime?.getMinutes().toString().padStart(2, "0")
			);
		}
